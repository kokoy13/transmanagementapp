...
<form
    ...
    class="space-y-6"
    x-data="{
        selectedId: '{{ old('id', $order->customer->id ?? '') }}',
        selectedAddress: '{{ old('address', $order->installation_address ?? '') }}',
        selectedPacket: '{{ old('packet', $order->packet->name ?? '') }}',
        selectedBandwidth: '{{ old('bandwidth', $order->packet->bandwidth ?? '') }}',
...
>
    ...
    <div>
        ...
        <select
            ...
            required>
            <option value="" disable>Pilih ID Customer</option>
            @forelse ($customers as $customer)
                <option value="{{ $customer->id }}" {{ $customer->id }} - {{ old('id, $order->customer->id' ==  $customer->id ? 'select' : '')}}>
                    {{ $customer->id }} - {{ $order->customer->full_name ?? 'Tanpa Nama' }}
                </option>
            @empty
    ...
    <div>
        ...
        <textarea
            ...
            required
        >{{ old('address', $order->installation_address ?? '') }}</textarea>
        @error('address')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        @enderror
