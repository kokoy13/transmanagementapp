...
class PaymentController extends Controller
{
    ...
    public function update(PaymentRequest $request, $id)
    {
        ...
        if ($request->hasFile('transaction_reference')) {
            if ($payment->transaction_reference && Storage::disk('public')->exists('payments/' . $payment->transaction_reference)) {
                Storage::disk('public')->delete('payments/' . $payment->transaction_reference);
            }
            $store = $request->file('transaction_reference')->store('public/payments');
            $validated['transaction_reference'] = basename($store);
        }
